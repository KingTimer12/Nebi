...
const { mainMessagesForum } = require("../messages/forumAppMessage.js");
  ...
  if (answers.length) {
    for (const row of answers) {
      ...
      await forumChannel.threads
        .create({
          name: `${row.answer[2]}`,
          message: { content: `<@!${userId}>`, embeds: mainMessagesForum(userId, row.answer, row.data, questions) },
        ...
        .then(async (threadChannel) => {
          const msgIds = threadChannel.messages.cache
            ...
            .map((msg) => msg.id);
          await sendApp(userId, msgIds.at(0), row.data, row.answer);
        });
    }
    answers = [];
  }
